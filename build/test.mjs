import { ArticleApi,
// CommentApi, Space, SpaceApi, Tag, TagApi, TextApi, Text, PageApi, KeyValueApi
 } from "./index.mjs";
// import { Nullable } from "./types.mjs";
import util from 'util';
const logData = (data) => console.log(util.inspect(data, {
    showHidden: false,
    depth: null,
    colors: true
}));
...
// }, (data) => logData(data));
// logData(sArticles);
const articleId = await (new ArticleApi({ spaceId, apiKey })).importArticle({
    files: [],
    tags: [],
    name: '10th import article',
    published: true,
...
logData(articleId);
const articleIdV = await (new ArticleApi({ spaceId, apiKey })).importArticleVersion({
    parentId: articleId,
    country: ['sk'],
    description: 'descr',
    keywords: 'keys',
    language: 'en',
    ogDescription: '8ogDescri',
    ogImage: 'ogIma',
    ogTitle: 'ogTit',
