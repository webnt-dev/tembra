function t(t,...e){return t.reduce((t,a,i)=>`${t}${a}${i in e?e[i]:""}`,"")}function e(t,e){return{data:null,errors:[{message:"",locations:[],path:[]}],http:{status:e,statusText:t}}}async function a(t,a,i={},s){try{const r=await fetch(t,{method:"POST",headers:i,body:JSON.stringify({query:a.query,variables:a.variables})});if(200!==r.status)return e(r.statusText,r.status);const n=await r.json();return n.http={status:200,statusText:"OK"},Array.isArray(n.errors)||(n.errors=null),n.data||(n.data=null),s&&s(n),n}catch(t){const a=e("",0);return t instanceof Error&&(a.errors[0].message=t.name,"AbortError"===t.name&&(a.http.status=1)),a}}class i extends Error{data;constructor(t){super(t.extensions?.code??t.message),this.data=t.extensions?.data??{},this.name="GraphQLError"}}function s(t){if(t.errors?.length)throw new i(t.errors[0])}const r="https://api.tembra.app";class n{config;apiUrl;constructor(t,e){this.config=t,this.apiUrl=e}async ping(e){const i=await a(this.apiUrl,{query:t`
			...
			`},{"x-wnt-space-id":this.config.spaceId});return s(e),e.data?.get??null}}const l=`${r}/public/cms/graphql/article`;class p extends n{config;constructor(t){super(t,l),this.config=t}async listTags(e){const i=await a(this.apiUrl,{query:t`
				query {
					listTags {
						id
						name
			...
			`},{"x-wnt-space-id":this.config.spaceId},e);return s(i),i.data?.listTags??[]}async importTag(e,i){const r=await a(this.apiUrl,{query:t`
				...
				`,variables:{data:e}},{"x-wnt-space-id":this.config.spaceId,"x-wnt-api-key":this.config.apiKey??""},i);return s(r),r.data?.importTag??""}}function u(t){return{...t,createdAt:new Date(t.createdAt),updatedAt:new Date(t.updatedAt),publishedFrom:new Date(t.publishedFrom),publishedTo:t.publishedTo?new Date(t.publishedTo):null}}class c extends n{config;constructor(t){super(t,l),this.config=t}async getArticle(e,i,r){const n=await a(this.apiUrl,{query:t`
			...
			`,variables:{id:e,ver:i}},{"x-wnt-space-id":this.config.spaceId},r);if(s(n),n.data?.getArticle){const t=n.data?.getArticle;return{...u(t),versions:t.versions.map(t=>({...u(t)})),fullVersions:t.fullVersions?.map(t=>({...u(t)}))??void 0}}return null}async listArticles(e,i,r){const n=await a(this.apiUrl,{query:t`
			...
			`,variables:{data:e,ver:i}},{"x-wnt-space-id":this.config.spaceId},r);s(n);const d=n.data?.listArticles;return d.map(t=>({...u(t),versions:t.versions.map(t=>({...u(t)})),fullVersions:t.fullVersions?.map(t=>({...u(t)}))??void 0}))}async searchArticleVersions(e,i){const r=await a(this.apiUrl,{query:t`
			...
			`,variables:{data:e}},{"x-wnt-space-id":this.config.spaceId},i);s(r);const n=r.data?.searchArticleVersions;return{...n,items:n.items.map(t=>({...t,...u(t)}))}}async importArticle(e,i){const r=await a(this.apiUrl,{query:t`
				mutation importArticle($data: ImportArticleInput!) {
					importArticle(data: $data)
				}
			`,variables:{data:{...e,publishedFrom:e.publishedFrom.toISOString(),publishedTo:e.publishedTo?.toISOString()??null}}},{"x-wnt-space-id":this.config.spaceId,"x-wnt-api-key":this.config.apiKey??""},i);return s(r),r.data?.importArticle??""}async importArticleVersion(e,i){const r=await a(this.apiUrl,{query:t`
				mutation importArticleVersion($data: ImportArticleVersionInput!) {
					importArticleVersion(data: $data)
				}
			`,variables:{data:e,publishedFrom:e.publishedFrom.toISOString(),publishedTo:e.publishedTo?.toISOString()??null}},{"x-wnt-space-id":this.config.spaceId,"x-wnt-api-key":this.config.apiKey??""},i);return s(r),r.data?.importArticleVersion??""}}class g extends n{config;constructor(t){super(t,l),this.config=t}async createComment(e,i){const r=await a(this.apiUrl,{query:t`
			...
			`,variables:{data:e}},{"x-wnt-space-id":this.config.spaceId},i);return s(r),r.data?.listComments}}const m=`${r}/public/cms/graphql/page`;function h(t){return{...t,createdAt:new Date(t.createdAt),updatedAt:new Date(t.updatedAt),publishedFrom:new Date(t.publishedFrom),publishedTo:t.publishedTo?new Date(t.publishedTo):null}}class y extends n{config;constructor(t){super(t,m),this.config=t}async importPage(e,i){const r=await a(this.apiUrl,{query:t`
			...
			`,variables:{data:e}},{"x-wnt-space-id":this.config.spaceId},i);s(r);const n=r.data?.listPageVersions;return n.map(h)}}const A=`${r}/public/cms/graphql/text`;class b extends n{config;constructor(t){super(t,A),this.config=t}async importCategory(e,i){const r=await a(this.apiUrl,{query:t`
			...
			`,variables:{data:e,ver:i}},{"x-wnt-space-id":this.config.spaceId},r);s(n);const d=n.data?.listTexts;return d.map(t=>({...t,createdAt:new Date(t.createdAt),updatedAt:new Date(t.updatedAt),versions:t.versions.map(t=>({...t,createdAt:new Date(t.createdAt),updatedAt:new Date(t.updatedAt)})),fullVersions:t.fullVersions?.map(t=>({...t,createdAt:new Date(t.createdAt),updatedAt:new Date(t.updatedAt)}))??void 0}))}}const I=`${r}/public/cms/graphql/keyValue`;function w(t){return{...t,createdAt:new Date(t.createdAt),updatedAt:new Date(t.updatedAt)}}class f extends n{config;constructor(t){super(t,I),this.config=t}async importKeyValue(e,i){const r=await a(this.apiUrl,{query:t`
			...
			`,variables:{data:e,ver:i}},{"x-wnt-space-id":this.config.spaceId},r);s(n);const d=n.data?.listKeyValues;return d.map(t=>({...w(t),versions:t.versions.map(w),fullVersions:t.fullVersions?.map(w)??void 0}))}}export{c as ArticleApi,g as CommentApi,f as KeyValueApi,y as PageApi,o as SpaceApi,p as TagApi,b as TextApi};
//# sourceMappingURL=tembra-api.1.0.1.mjs.map
